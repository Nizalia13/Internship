...
\section{\texorpdfstring{$\hat{t}$}{t̂} Calculation}
\label{sec3.5: t hat calculation}

\subsection{\texorpdfstring{$\tau$ Computation}{tau Computation}}
\label{3.5.1: computation}
For any test point $\mathbf{s} \in \mathbb{R}^K$, we define the inclusion threshold function $\tau(\mathbf{s})$ (defined 
...
Given the computed $\tau$ values over the scaling set $S_2$, the calibrated scaling factor $\hat{t}$ is selected as 
...
Unlike the original CSA method, which relies on a computationally expensive iterative binary search over candidate 
...
\subsection{Class Specific \texorpdfstring{$\hat{t}$}{t̂} Calculation}
\label{sec3.5.2: class specific t hat}
When the dataset has class imbalance (like our dataset), a single global scaling factor $\hat{t}$ can cause under 
...
The two prediction tasks in this report differ in how envelopes are constructed, which determines how these per 
class thresholds $\hat{t}_c$ are applied:

\textbf{Gram Type Classification: } We only fit a single envelope, $E$ on the shared shape discovery set $S_1$ 
...
Since $\hat{t} \ge \hat{t}_c$ for both classes, scaling $E$ by $\hat{t}$ satisfies both coverage requirements 
...
\textbf{Host Level Classification: } In host-level classification, each candidate host $h \in \mathcal{H}$ has a 
...
\subsection{Resolving the Empty Prediction Set}
\noindent In section~\ref{sec3.1: problem formalisation} we noted that $C(\mathbf{s},\mathbf{m})$ may be empty when no 
candidate label's transformed score falls inside $\hat t \cdot E$. In both our tasks we resolve this differently.

For gram type classification, we resolve an empty set conservatively by returning both labels, $C(\mathbf{s}) = 
...
would be uninformative, we resolve an empty set by forcing a singleton at the host with the closest $\tau$ value:
\begin{equation*}
C(\mathbf{s}) = \Big\{ \operatorname*{argmin}_{h \in \hat{\mathcal{H}}(\mathbf{s})} \tau_h(\mathbf{s}_h) \Big\}.
\end{equation*}

Both apply only when the primary rule of Equation~\eqref{eq:prediction_set_definition} yields $\emptyset$.
Coverage guarantee in Theorem~\ref{thm: marginal coverage} remains unaffected as nonempty fallback options strictly 
enlarge $C(\mathbf{s})$, thus the lower bound $\mathbb{P}(H \in C(\mathbf{s})) \ge 1 - \alpha$ trivially holds. Consequently, 
the reported \texttt{empty\_rate} is strictly zero by rule rather than an absence of out-of-envelope points. 
