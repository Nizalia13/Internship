...
def evaluate_prediction_sets(prediction_sets, true_labels):
    ...
    covered = [true_labels[i] in prediction_sets[i] for i in range(len(true_labels))]

    sizes = [len(s) for s in prediction_sets]

    singleton_idx = [i for i, s in enumerate(prediction_sets) if len(s) == 1]

    # Per-class coverage
    classes = list(dict.fromkeys(true_labels))
    per_class_coverage = {}

    for cls in classes:
        idx = [i for i, y in enumerate(true_labels) if y == cls]

        class_covered = [true_labels[i] in prediction_sets[i] for i in idx]

        per_class_coverage[cls] = float(np.mean(class_covered))
