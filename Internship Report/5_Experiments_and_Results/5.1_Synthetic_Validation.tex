...
In this section we will present the experimental validation of the methods developed in Section~\ref{sec3: 
...
Before applying the conformal envelope constructions of Section~\ref{sec3.4: envelope construction} to real phage 
...
\subsection{2D Geometric Proof and Class Conditional Efficiency}
\label{sec5.1.1: 2d proof of concept}

We first simulated $N = 400$ bivariate score pairs from $\mathcal{N}((0.15,0.15), \Sigma)$ with 
...
Next, we introduced four classes ($\text{Hosts A--D}$) with centroids at $(0.1,0.7)$, $(0.7,0.1)$, $(0.1,0.1)$, 
...
Computing host specific scaling thresholds $\hat{t}_h$ while sharing the directional shape $\tilde{\mathbf{q}}$ 
...
Similarly, we also evaluated the non convex Radial and Strip methods on this same 2D synthetic dataset ($K=2$, $\alpha=0.10$). The 
results confirmed that they both successfully adapt to the negatively correlated score distribution, and achieve the exact target 
coverage of $90.0\%$ with and have size scaling factors of $\hat{t} = 0.996$ and $\hat{t} = 0.944$, respectively. These values are 
comparable to the convex baseline method, which has a $\hat{t} \approx 1$ on this dataset and is also consistent with the coverage 
results reported for all three methods in Section~\ref{sec5.1.2: dimensionality scaling}.
...
\subsection{Dimensionality Scaling and Finite-Sample Coverage}
\label{sec5.1.2: dimensionality scaling}

To evaluate coverage validity as the dimensionality grows, we benchmarked the reference CSA paper method (binary 
...
\centering
\small
\setlength{\tabcolsep}{3pt} % Adjust spacing between columns
\resizebox{\textwidth}{!}{%
...
\textbf{Dimension ($K$)} & \textbf{Coverage (\%)} & $\hat{\mathbf{t}}$ & \textbf{Coverage (\%)} & $\hat{\mathbf{t}}$ & 
\textbf{Coverage (\%)} & $\hat{\mathbf{t}}$ \\
\midrule
...
coverage and calibrated size-scaling factors $\hat{t}$ are reported across envelope geometries. Strip results at $K=20$ 
are omitted (--) because at fixed $N=1\,000$ its $M_{\text{bin}}=20$ bins per conditioning dimension leave too few points 
per bin for reliable quantile estimation as $K$ grows.}
\label{tab: dim scaling results}
\end{table}


As shown in Table~\ref{tab: dim scaling results}, every method achieves exactly $90.0\%$ empirical coverage across 
...
\subsection{Geometric Robustness Across Score Topologies}
\label{sec5.1.3: distribution robustness}

To assess the boundary adaptation under realistic non convex or misspecified feature spaces, we calibrated 
$3\text{D}$ ($K=3$) envelopes ($\alpha = 0.10, N = 400$) across four distinct score topologies: (i) \emph{Uncorrelated 
Gaussian} ($\Sigma = \mathbf{I}$), (ii) \emph{Correlated Gaussian} ($\rho = 0.85$), (iii) \emph{Multivariate 
...
\textbf{Score Distribution Topology} & \textbf{Coverage (\%)} & $\hat{\mathbf{t}}$ & \textbf{Coverage (\%)} & 
$\hat{\mathbf{t}}$ \\\\
\midrule
...
The results in Table~\ref{tab: 3d topology results} highlight several key properties:
\begin{itemize}
    ...
    \item \textbf{Heavy-Tail Sensitivity:} The heavy tailed Laplace distribution induces the largest inflation factor for 
           the Strip envelope ($\hat{t} = 1.232$). This is because heavy tails produce extreme nonconformity values which 
           result in higher quantile thresholds during the shape discovery step.
\end{itemize}
...
\subsection{Multi-Host Benchmark Verification}
\label{sec5.1.4: multi host validation}

Finally, we validate the per-host envelope cascade (Section~\ref{sec3.1: problem formalisation}) on a simulated 
...
Across the reference CSA, Radial, and Strip constructions, all three methods correctly isolate $C(\mathbf{s}) = 
