...
\noindent We evaluated the three envelope geometries (Radial, Strip, and Collapsed 1D) on the real phage data for a binary 
Gram type classification ($\mathcal{H} = \{\text{gram-neg}, \text{gram-pos}\}$). We only used the 
$4433$ phages in split 0 to ensure that all scores remain out of fold by construction and there is no contamination.


\subsubsection{Experimental Setup}
\label{sec5.2.1: gram setup}
\noindent We partitioned the split 0 subset into shape discovery ($|S_1| = 1\,768$), size scaling ($|S_2| = 1\,326$), and 
...
\noindent At test time, we evaluated the Gram type nonconformity transformation under both candidate hypotheses. For an empty 
...
\noindent As we can see in Figure~\ref{fig: gram results} (middle panels), all three envelope geometries achieve $99.8\%$ empirical 
...
\vspace{0.15cm}
\noindent We can also see that the high coverage does not compromise efficiency. The set size distribution chart of Figure~\ref{fig: 
...
\vspace{0.15cm}
\noindent Acheiving this coverage that is over the target highlights a trade off, it suggests the calibrated 
