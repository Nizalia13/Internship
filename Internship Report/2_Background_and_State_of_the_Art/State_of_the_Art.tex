...
host, $h \in \mathcal{H}$, as a varaible order Markov chain with parameter vector, $\theta_h$. It then computes the the likelihood 
of the new phage genome sequence, $S = (a_1, \cdots, a_L)$ under each host model:
$$\ell(S \mid \boldsymbol{\theta}_h) = \sum_{i=k+1}^L \log \mathbb{P}\left(a_i \mid a_{i-k}, \dots, a_{i-1}; 
\boldsymbol{\theta}_h\right)$$
The predicted host is $\hat{y} = \operatorname*{argmax}_{h \in \mathcal{H}} \ell(S \mid \boldsymbol{\theta}_h)$, 
...
maps this vector to the probability simplex over the host categories
$$f_{\text{RF}}: \{0,1\}^M \longrightarrow \Delta^{|\mathcal{Y}|-1}, \quad \text{where } \hat{\mathbf{p}} = 
f_{\text{RF}}(\mathbf{x}), \ \sum_{y \in \mathcal{Y}} \hat{p}_y = 1$$

% $f: \{0,1\}^M \to \varDelta(\mathcal{Y})$ 
% maps the binary feature vector to a probability distribution over the hosts $\mathcal{Y}$. 
...
amino acid alphabet of size 20. $\mathcal{A}$ is mapped by an encoder $\Phi_{\text{pLM}}$ (e.g., ProtT5-XL-UniRef50)
\cite{elnaggar2022prottrans} to a embedding matrix $$Z = \Phi_{\text{pLM}}(a) \in \mathbb{R}^{n \times d}$$. This is followed
by mean pooling along the sequence length which yields a fixed length embedding $z \in \mathbb{R}^d$, where $d$ is the embedding 
dimension (typically 1024 or larger). This embedding vector, $z$, captures the sequence'sbbiochemical, structural, and 
evolutionary properties. A primary foundational model in this domain is ProtTrans/ProtT5 \cite{elnaggar2022prottrans}, 
...
adjust the raw output scores of teh model so that they reflect the empirical probabilities better. For example, they make
sure that among all the predictions which are assigned the score of $0.8$, around $80\%$ should be correct. This is 
easy to apply on top of any existing models and is also compuationally cheap, but they do not provide us with any formal 
finite sample guarantees. The calibration is verified empirically on a validation set but it can degrade on new data with no
bounds on how far the calibrated probabilities may deviate from the truth on the unseen test set. 
